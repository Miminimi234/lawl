#!/usr/bin/env python3
"""
Standalone VERDICT server - Runs without Docker
Pulls REAL cases from CourtListener API
Uses OpenAI GPT-4 for comprehensive legal analysis
Run with: python3 standalone_server.py
Set COURTLISTENER_TOKEN and OPENAI_API_KEY env vars
"""
...
# Import REAL CASE SCRAPERS (NO API KEYS NEEDED!)
try:
    from app.services.supreme_court_scraper import get_real_cases_mix, SupremeCourtScraper
    REAL_CASE_SCRAPER_AVAILABLE = True
    print("✅ Supreme Court & Federal Court scraper loaded")
except ImportError as e:
    REAL_CASE_SCRAPER_AVAILABLE = False
    print(f"⚠️  Real case scraper not available: {e}")

# AI Analyzer availability check
AI_ANALYZER_AVAILABLE = False  # Not using AI for case generation in this mode

app = FastAPI(title="Verdict API - Standalone")
...
# In-memory database with realistic cases
CASES_DB = []
CASE_ID_COUNTER = 1
...
async def fetch_real_cases():
    """Scrape REAL cases from public courts - NO API needed!"""
    global CASE_ID_COUNTER
    
    if not REAL_CASE_SCRAPER_AVAILABLE:
        print("   ⚠️  Real case scraper not initialized")
        return 0
    
    print("\n📡 Scraping REAL federal court cases...")
    print("   🏛️  Supreme Court + Federal Circuits • NO signup • Completely FREE!")
    
    try:
        # Use the imported get_real_cases_mix function
        real_cases = get_real_cases_mix(limit=50)
        
        if not real_cases:
            print("   ⚠️  No cases returned from scraping")
            return 0
        ...
        for raw_case in real_cases:
            ...
            existing = next((c for c in CASES_DB if c.get('title') == raw_case.get('title')), None)
            if existing:
                continue
            
            # Determine case type from title/court
            case_type = 'general'
            title_lower = raw_case.get('title', '').lower()
            if any(word in title_lower for word in ['contract', 'breach', 'agreement']):
                case_type = 'contract'
            elif any(word in title_lower for word in ['employ', 'discrimin', 'labor']):
                case_type = 'employment'
            elif any(word in title_lower for word in ['civil rights', 'constitutional', 'amendment']):
                case_type = 'civil_rights'
            elif any(word in title_lower for word in ['property', 'land', 'real estate']):
                case_type = 'property'
            elif any(word in title_lower for word in ['criminal', 'prosecution', 'people v', 'state v']):
                case_type = 'criminal'
            
            case = {
                "id": CASE_ID_COUNTER,
                "case_number": raw_case.get('citation', f"REAL-{CASE_ID_COUNTER:05d}"),
                "title": raw_case.get('title', 'Unknown Case'),
                "jurisdiction": raw_case.get('jurisdiction', 'Federal Court'),
                "case_type": case_type,
                "status": "completed",
                "facts": f"""REAL SUPREME COURT CASE

{raw_case.get('title')}
{raw_case.get('citation', 'N/A')}
Decided by: {raw_case.get('court', 'Federal Court')}

This is an actual case from the United States court system. Full opinion available at: {raw_case.get('url', 'N/A')}

[Case facts and reasoning available in the official court opinion]""",
                "recommendation": f"★ REAL CASE ★ {raw_case.get('citation', raw_case.get('title'))}",
                "confidence": 1.0,
                "created_at": datetime.now().isoformat(),
                "analysis": {
                    "judge_analyses": [{
                        "judge_name": f"Official Opinion - {raw_case.get('court', 'Federal Court')}",
                        "specialty": raw_case.get('jurisdiction', 'Federal Law'),
                        "framework_used": "actual_judicial_opinion",
                        "reasoning": f"""★★★ REAL COURT CASE ★★★

This is NOT a generated case or AI analysis. This is an ACTUAL judicial opinion from:

Court: {raw_case.get('court', 'Federal Court')}
Citation: {raw_case.get('citation', 'N/A')}
Year: {raw_case.get('year', 'Recent')}

Case Title: {raw_case.get('title')}

This case represents actual legal precedent from the United States court system. The opinion was written by real federal judges and has binding or persuasive authority.

To read the full official opinion, visit:
{raw_case.get('url', 'Available through public court records')}

[This entry serves as a reference to the real case. Full legal analysis can be found in the official court opinion.]
""",
                        "recommendation": "★ REAL JUDICIAL OPINION (not AI-generated)",
                        "confidence": 1.0
                    }],
                    "consensus": {
                        "final_verdict": f"★ REAL CASE: {raw_case.get('citation', raw_case.get('title'))}",
                        "reasoning": f"This is an actual court opinion from {raw_case.get('court', 'federal court')}, not an AI-generated analysis. This case represents binding legal precedent.",
                        "agreement_score": 1
                    }
                },
                "source": "real_court_opinion",
        ...
        print(f"\n🎉 Scraped {added_count} REAL court cases from public sources!\n")
        return added_count
        
    except Exception as e:
        print(f"   ❌ Error scraping real cases: {e}")
        import traceback
        traceback.print_exc()
        return 0
...
async def background_case_fetcher():
    ...
    while True:
        try:
            ...
            print("\n🔄 Scraping new real cases...")
            count = await fetch_real_cases()
            if count > 0:
...
async def startup_event():
    ...
    # Try to scrape real cases first
    count = await fetch_real_cases()
    
    # If no real cases, load mock data
    if count == 0:
        print("\n⚠️  Could not scrape real cases. Loading mock data...\n")
        load_mock_cases()
    
    # Start background fetcher
    if REAL_CASE_SCRAPER_AVAILABLE:
        asyncio.create_task(background_case_fetcher())
        print("🔄 Background scraper started (checks for new cases every hour)\n")
    ...
    if count > 0:
        print(f"   🏛️  Source: REAL court opinions (Supreme Court + Federal Circuits)")
    else:
        print(f"   📝 Source: Mock data (comprehensive analysis)")
